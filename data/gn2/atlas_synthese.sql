-- Creation d'une vue permettant de reproduire le contenu de la table du même nom dans les versions précédentes

CREATE OR REPLACE VIEW synthese.syntheseff as
WITH areas AS (
        SELECT DISTINCT ON (sa.id_synthese, t.type_code) 
            sa.id_synthese,
            sa.id_area,
            a.centroid,
            st_transform(a.centroid, 4326) AS centroid_4326,
            t.type_code
           FROM synthese.cor_area_synthese sa
             JOIN ref_geo.l_areas a ON sa.id_area = a.id_area
             JOIN ref_geo.bib_areas_types t ON a.id_type = t.id_type
          WHERE t.type_code::text = ANY (ARRAY['M10'::character varying, 'COM'::character varying, 'DEP'::character varying]::text[])
        ), obs_data AS (
        SELECT 
            s.id_synthese,
            s.cd_nom,
            s.id_dataset,
            s.date_min AS dateobs,
            s.observers AS observateurs,
            (s.altitude_min + s.altitude_max) / 2 AS altitude_retenue,
                CASE
                    WHEN ns.cd_nomenclature::text = '1'::text THEN ( SELECT a.centroid_4326
                       FROM areas a
                      WHERE a.id_synthese = s.id_synthese AND a.type_code::text = 'COM'::text
                     LIMIT 1)
                    WHEN ns.cd_nomenclature::text = '2'::text THEN ( SELECT a.centroid_4326
                       FROM areas a
                      WHERE a.id_synthese = s.id_synthese AND a.type_code::text = 'M10'::text
                     LIMIT 1)
                    WHEN ns.cd_nomenclature::text = '3'::text THEN ( SELECT a.centroid_4326
                       FROM areas a
                      WHERE a.id_synthese = s.id_synthese AND a.type_code::text = 'DEP'::text
                     LIMIT 1)
                    ELSE st_transform(s.the_geom_point, 4326)
                END AS the_geom_point,
            s.count_min AS effectif_total,
				CASE
					WHEN ns.cd_nomenclature = '0' THEN '5'
					WHEN ns.cd_nomenclature = '1' THEN '1'
					WHEN ns.cd_nomenclature = '2' THEN '2'
					WHEN ns.cd_nomenclature = '3' THEN '3'
					WHEN ns.cd_nomenclature = '4' THEN '4'
				END AS  diffusion_level
        FROM synthese.synthese s
             LEFT JOIN synthese.t_nomenclatures st ON s.id_nomenclature_observation_status = st.id_nomenclature
             LEFT JOIN synthese.t_nomenclatures ns ON s.id_nomenclature_sensitivity  = ns.id_nomenclature
			 LEFT JOIN synthese.t_nomenclatures vl ON s.id_nomenclature_valid_status =  vl.id_nomenclature
        WHERE  ns.cd_nomenclature::text <> '4'::text and st.cd_nomenclature::text = 'Pr'::text AND vl.cd_nomenclature NOT IN ('0','3','4','5')
        )
SELECT d.id_synthese,
    d.id_dataset,
    d.cd_nom,
    d.dateobs,
    d.observateurs,
    d.altitude_retenue,
    d.the_geom_point,
    d.effectif_total,
    c.insee,
    d.diffusion_level
   FROM obs_data d
     JOIN atlas.l_communes c ON st_intersects(d.the_geom_point, c.the_geom)