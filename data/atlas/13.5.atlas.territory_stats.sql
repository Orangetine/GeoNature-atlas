...
SELECT                                                                                     
	...
	COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.patrimonial = 'oui') AS nb_taxon_patrimonial,
	COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.protection_stricte = 'oui') AS nb_taxon_protege,
	COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.threatened = true and ta.cd_sig = 'INSEER11') AS nb_taxon_threatened,	
    area.description
...
LEFT JOIN atlas.vm_l_areas area ON area.id_area = ta.id_area
...
SELECT
    ...
    COUNT(DISTINCT ta.cd_ref) AS nb_species,
    ta.group2_inpn,
    COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.patrimonial = 'oui') AS nb_patrimonial,
	COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.protection_stricte = 'oui') AS nb_taxon_protege,
	COUNT(DISTINCT ta.cd_ref) FILTER (WHERE ta.threatened = true and ta.cd_sig = 'INSEER11') AS nb_taxon_threatened,
    (SELECT COUNT(*)
     FROM atlas.vm_taxons taxon
     WHERE taxon.group2_inpn = ta.group2_inpn) AS nb_species_in_teritory
FROM atlas.vm_taxons_areas ta
LEFT JOIN atlas.vm_l_areas area ON area.id_area = ta.id_area
JOIN atlas.vm_bib_areas_types AS bat ON  bat.id_type = area.id_type
WHERE bat.type_code = ANY(SELECT * from string_to_table(:type_code, ','))
GROUP BY ta.id_area, ta.group2_inpn
WITH DATA;
